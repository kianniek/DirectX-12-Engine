...
namespace Engine {
	...
	void DXGISwapChain::Initialize(ID3D12Device* pDevice, IDXGIFactory2* pFactory, ID3D12CommandQueue* pCommandQueue, const HWND hwnd, const UINT width, const UINT height)
	{
		...
		KOSMO_EVAL_HR(pDevice->CreateDescriptorHeap(&rtvHeapDesc, IID_PPV_ARGS(mRTVHeap.GetAddressOf())), "Error creating the RTV heap");
		...
		KOSMO_EVAL_HR(pFactory->CreateSwapChainForHwnd(pCommandQueue, hwnd, &description, nullptr, nullptr, GetAddressOf()), "Error creating swapchain");
	...
	void DXGISwapChain::CreateBuffers()
	{
		...
		for (int i = 0; i < mBufferCount; i++) {
			...
			KOSMO_EVAL_HR(Get()->GetBuffer(i, IID_PPV_ARGS(mRenderTargets[i].GetAddressOf())), "Error creating buffer for swapchain idx: " << i);
